    ...
    if (!player) {
        return (
            <div className="text-center py-20">
                ...
                <button onClick={() => navigate('/rankings')} className="mt-4 text-blue-400 hover:underline">Back to Rankings</button>
            </div>
        )
    }
        ...
        >
            ...
            <div className="bg-slate-800 rounded-2xl p-6 shadow-xl border border-slate-700/50">
                <div className="md:flex items-start md:space-x-8">
                    ...
                    <div className="flex-shrink-0 text-center md:text-left mb-6 md:mb-0">
                        <motion.div
                            className="w-32 h-32 md:w-40 md:h-40 mx-auto md:mx-0 rounded-full bg-slate-700 border-4 border-slate-600 overflow-hidden shadow-2xl relative"
                    ...
                    <div className="flex-grow space-y-4">
                        <div className="text-center md:text-left">
                            ...
                            <div className="flex items-center justify-center md:justify-start space-x-4 text-slate-400">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
                            <StatBox label="Current Rank" value={`#${player.rank}`} icon={<Crown className="w-5 h-5 text-yellow-400" />} />
                            <StatBox label="Best Rank" value={`#${player.bestRank}`} icon={<TrendingUp className="w-5 h-5 text-blue-400" />} />
                            <StatBox label="Career Titles" value={player.careerTitles} icon={<Trophy className="w-5 h-5 text-green-400" />} />
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700/50">
                    ...
                    <p className="mt-4 text-sm text-slate-400">Last 5 matches</p>
                </div>
                ...
                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700/50">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-400">Sets Won/Lost</span>
                            <span className="text-white font-mono">{player.sw} / 24</span>
                        </div>
                        <div className="w-full bg-slate-700 h-2 rounded-full overflow-hidden">
                            <div className="bg-blue-500 h-full" style={{ width: `${(player.sw / (player.sw + 24)) * 100}%` }}></div>
                        </div>
                        <div className="flex justify-between items-center text-sm pt-2">
                            <span className="text-slate-400">Game Difference</span>
                            <span className="text-green-400 font-mono">+{player.gd}</span>
                        </div>
                    </div>
                </div>
            </div>
